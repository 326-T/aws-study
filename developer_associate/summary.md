...
DynamoDB StreamsはStreamを保存できるわけではなくてS3のイベント通知と同じ
...
API Gatewayで「環境を動的に切り替える」ための構成を、極限まで削ぎ落として整理。
...
## AWS SDK for Python (Boto3) はアクセスキーとシークレットアクセスキーでのアクセスのみ

IAMユーザ名とパスワードは無理。それはそうか

## InvalidSignatureException – 時計を疑え
...
既存のロググループを変更すれば暗号化できる

鍵は対称キー

非対称キーはかなり重く、一部のストレージでしか使われない

## CloudWatch エージェントはEC2だけでなくオンプレもいける
...
## CodeBuildとCodeDeployの設定ファイルの置き場
