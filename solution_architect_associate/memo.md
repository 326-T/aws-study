...
はい、おっしゃる通りです。OSレベルのループバックアドレス（`127.0.0.1`）とは別に、AWSという「クラウドのインフラ層」にアクセスするための**固定（マジック）IPアドレス**と、ネットワーク設計で使えない**予約アドレス**が存在します。

SAA/SAP試験で「これが出てきたらアレだ」と即答すべきものを列挙します。
